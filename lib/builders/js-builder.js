...
module.exports = class JsBuilder extends BaseBuilder {
    ...
    _build(venderast, bundle) {
        return co(function *() {
			...
			const buildResult = uglify.minify(bundle.sources, uglifyOpts);

			// CREATE A DEST DIRECTORY IF NOT EXISTS
			yield fs.ensureDir(venderast._dest);

			// WRITE CODE
			yield fs.writeFile(bundle.path, buildResult.code, 'utf8');

			// WRITE SOURCEMAPS
			if (venderast._sourcemaps) {
				yield fs.writeFile(bundle.path + '.map', buildResult.map, 'utf8');
			}
        });
    }
};

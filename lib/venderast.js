	...
	bundle(bundleFileName, forceRebuild) {
		...
		return co(function *() {
			...
			const sourceHasChanges = stats.some(result => {
				const cacheValue = self._cache.get(result.path);
			...
			stats.forEach(result => {
				if (result.stat === null) self._cache.remove(result.path);
				else self._cache.set(result.path, result.stat.mtime.toJSON());
			});
